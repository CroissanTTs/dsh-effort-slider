...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 取当前模型可用的档位列表。directory store 快照形状是
		 * {current, routable, groups, failures, status, error}：选择在 current，
		 * 目录在 groups（每组 {id, name, models:[{id, name, reasoning:{efforts:[...]}}]}）。
		 */
		...
		// directoryFor 内部直接访问 ctx.sessions / ctx.remote.session / ctx.get("conversation")，
		// 这些是 cordis 服务，必须在 inject 里声明，否则 scope 里拿不到会抛
