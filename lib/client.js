...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = `
.dsh-rs{display:flex;align-items:center;gap:8px;height:28px;padding:0 10px;min-width:0;flex:none}
.dsh-rs-track{position:relative;width:150px;flex:none;height:20px;display:flex;align-items:center;touch-action:none}
...
.dsh-rs-label{flex:none;width:56px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px;font-weight:600;line-height:18px;color:var(--dsw-alias-label-secondary);text-align:right}
.dsh-rs-cap{flex:none;font-size:12px;color:var(--dsw-alias-label-caption);white-space:nowrap}`;
		...
		function ReasoningSlider({ store, select }) {
			...
			// React 的 onChange 对 range 是逐像素触发（不是松手），所以提交改到
			// onPointerUp/onKeyUp/onTouchEnd——只在松手/按键抬起时写 store，
			// 拖动期间不触发 directory.select，官方选择器不再被带着重排。
		...
		function apply(ctx) {
			ctx.inject(["slots", "modelDirectories"], (scope) => {
				...
				// conversation.input.left 是 list slot（session 作用域，底部左侧），与官方控件并存、不冲突。
