...
.dsh-rsi-dot{width:8px;height:8px;border-radius:50%;background:var(--dsw-alias-label-primary);flex:none;transition:background .25s}
...
.dsh-rs-track{position:relative;width:180px;flex:none;height:14px;border-radius:999px;background:var(--dsw-alias-border-l2);overflow:hidden;touch-action:none}
...
.dsh-rsi-levels{display:flex;justify-content:space-between;gap:2px}
.dsh-rsi-level{font-size:10px;color:var(--dsw-alias-label-tertiary);cursor:pointer;padding:2px 4px;border-radius:6px;white-space:nowrap;transition:color .12s,background .12s}
.dsh-rsi-level:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}
.dsh-rsi-level.active{color:var(--dsw-alias-label-primary);font-weight:600}`;
		...
		function ReasoningSlider({ store, select }) {
			...
			return jsxs("div", { className: "dsh-rsi", ref, children: [
				...
				open && jsxs("div", { className: "dsh-rsi-pop", children: [
					...
					jsx("div", { className: "dsh-rsi-levels", children: levels.map((l) => jsx("span", {
						key: l.id,
						className: `dsh-rsi-level${l.id === current ? " active" : ""}`,
						onClick: () => { if (select) select({ provider, model, reasoningEffort: l.id }); setOpen(false); },
						children: l.name
					})) })
				]})
			] });
