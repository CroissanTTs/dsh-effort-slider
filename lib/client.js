...
.dsh-rs-track{position:relative;width:100%;flex:none;height:18px;border-radius:999px;background:var(--dsw-alias-border-l2);overflow:hidden;touch-action:none}
.dsh-rs-range{position:absolute;left:9px;right:9px;top:0;bottom:0}
.dsh-rs-fill{position:absolute;left:-2px;top:0;bottom:0;border-radius:999px;background:var(--dsw-static-deepseek-100,#e4edfd);transition:width .15s ease}
...
.dsh-rs-thumb{position:absolute;left:0;top:50%;width:18px;height:18px;border-radius:50%;background:var(--dsw-alias-state-business-primary);box-shadow:0 0 0 3px var(--dsw-alias-bg-module-platform),0 1px 3px rgba(0,0,0,.25);transform:translate(-50%,-50%);transition:left .15s ease;pointer-events:none}
